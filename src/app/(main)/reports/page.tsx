...
import { Chart as ChartJS, CategoryScale, LinearScale, BarElement, Title, Tooltip, Legend, type ChartOptions, registerables } from 'chart.js';
...
export default function Reports() {
    ...
    function changeWeek(forward: boolean) {
        ...
        // Set up the relevant date window
        let monday = new Date(labels[0]!.getTime() + 7 * 24 * 60 * 60_000 * (forward ? 1 : -1)) // TODO: Sus
        while (monday.getDay() !== 1) {
            monday = new Date(monday.getTime() - 86400000)
        }
        for (let i = 0; i < 7; i++) {
    ...
    for (const project of user.subjects) {
        ...
        for (const myDay of labels) {
            const sameDayLogs = projectLogs.filter(idk => isSameDay(new Date(idk.startedAt), new Date(myDay)))
            let x = 0
            for (const sameDayLog of sameDayLogs) { x += sameDayLog.duration }
            my_data.push(x / 60)
        }
        // TODO: Type checking here
        my_datasets.push({ label: project.name, data: my_data, backgroundColor: backgroundColours[project.colour as ColourType], borderColor: borderColours[project.colour as ColourType] })
    }
